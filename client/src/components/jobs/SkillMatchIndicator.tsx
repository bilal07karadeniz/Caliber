...
interface Props { skillName: string; requiredLevel?: number; userLevel?: number; }
...
export default function SkillMatchIndicator({ skillName, requiredLevel, userLevel }: Props) {
  ...
  return <Badge variant={matched ? 'success' : partial ? 'warning' : 'error'}>{skillName}{requiredLevel ? ` L${requiredLevel}` : ''}</Badge>;
}
